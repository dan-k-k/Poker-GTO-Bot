# app/trainingL1/training_utils.py
...
class TrainingUtils:
    ...
    def ensure_models_dir():
        ...
        if not os.path.exists(TrainingUtils.MODELS_DIR):
            os.makedirs(TrainingUtils.MODELS_DIR)
            print(f"📁 Created models directory: {TrainingUtils.MODELS_DIR}")
        return TrainingUtils.MODELS_DIR
    ...
    def load_existing_models(avg_pytorch_net, br_pytorch_net):
        ...
        avg_model_path = os.path.join(TrainingUtils.MODELS_DIR, "avg_pytorch_net.pt")
        br_model_path = os.path.join(TrainingUtils.MODELS_DIR, "br_pytorch_net.pt")
        ...
        if os.path.exists(avg_model_path):
            try:
                ...
                print(f"✅ Loaded existing average strategy model from {TrainingUtils.MODELS_DIR}/")
        ...
        if os.path.exists(br_model_path):
            try:
                ...
                print(f"✅ Loaded existing best response model from {TrainingUtils.MODELS_DIR}/")
            except Exception as e:
    ...
    def save_models(avg_pytorch_net, br_pytorch_net, current_episode):
        """Save both networks to the models directory."""
        TrainingUtils.ensure_models_dir()
        avg_path = os.path.join(TrainingUtils.MODELS_DIR, "avg_pytorch_net.pt")
        br_path = os.path.join(TrainingUtils.MODELS_DIR, "br_pytorch_net.pt")
        torch.save(avg_pytorch_net.state_dict(), avg_path)
        torch.save(br_pytorch_net.state_dict(), br_path)
        print(f"💾 Saved models to {TrainingUtils.MODELS_DIR}/ (episode {current_episode})")
